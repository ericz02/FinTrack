...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const totalAccounts = accounts.reduce((sum, account) => sum + (parseFloat(account.balance) || 0), 0);
    ...
    const renderExpensesPieChart = () => {
        ...
        return <Plot data={pieData} layout={{ title: 'Expenses by Category', width: 400, height: 400 }} />;
    };
    ...
    const renderExpensesBarChart = () => {
        ...
        const months = Object.keys(grouped).map(month => new Date(0, month).toLocaleString('default', { month: 'long' }));
    ...
    const renderDebtSummary = () => {
        const totalDebt = data.debts.reduce((sum, debt) => sum + (parseFloat(debt.amount) || 0), 0);
        return (
    ...
    return (
        <div className="container mx-auto p-4">
            <ToastContainer />
            <h1 className="text-2xl font-bold text-center mb-6">Financial Dashboard</h1>
